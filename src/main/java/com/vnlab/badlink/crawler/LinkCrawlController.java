 ...
 */
public class LinkCrawlController extends Configurable {

  static final Logger logger = LoggerFactory.getLogger(LinkCrawlController.class);
   ...
   */
  protected List<String> linkList = new ArrayList<String>();
  ...
  public LinkCrawlController(CrawlConfig config, PageFetcher pageFetcher, RobotstxtServer robotstxtServer)
      throws Exception {
    super(config);
  ...
  protected <T extends WebCrawler> void start(final WebCrawlerFactory<T> crawlerFactory, final int numberOfCrawlers, boolean isBlocking) {
    try {
      ...
      final LinkCrawlController controller = this;
                ...
                if (!someoneIsWorking) {
                  ...
                  if (!someoneIsWorking) {
                    ...
                    for (T crawler : crawlers) {
                      ...
                      Object object = crawler.getMyLocalData();
                      if (object != null && object instanceof LinkObject) {
	                      LinkObject linkObject = (LinkObject) object;
	                      if (learning == null || learning.assessObject(linkObject) == BLConstants.OUTPUT_TYPE) {
	                    	  logger.info("Accept Link: " + linkObject.getURL());
	                    	  linkList.add(linkObject.getURL());
	                      } else {
	                    	  logger.info("Discard link: " + linkObject.getURL());
	                      }
                      }
                    }
  ...
  public <T extends WebCrawler> void startWithBLCrawler(final int numberOfCrawlers, boolean isBlocking) {
	    try {
	      finished = false;
	      linkList.clear();
	      final List<Thread> threads = new ArrayList<>();
	      final List<BLCrawler> crawlers = new ArrayList<>();

	      for (int i = 1; i <= numberOfCrawlers; i++) {
	        BLCrawler crawler = new BLCrawler(this.learning);        
	        Thread thread = new Thread(crawler, "Crawler " + i);
	        crawler.setThread(thread);
	        crawler.init(i, this);
	        thread.start();
	              ...
	              while (true) {
	                ...
	                for (int i = 0; i < threads.size(); i++) {
	                  ...
	                  if (!thread.isAlive()) {
	                    if (!shuttingDown) {
	                      logger.info("Thread {} was dead, I'll recreate it", i);
	                      BLCrawler crawler = new BLCrawler(learning);
	                      thread = new Thread(crawler, "Crawler " + (i + 1));
	                      threads.remove(i);
	                      threads.add(i, thread);
	                      crawler.setThread(thread);
	                ...
	                if (!someoneIsWorking) {
	                  ...
	                  if (!someoneIsWorking) {
	                    ...
	                    // At this step, frontier notifies the threads that were waiting for new URLs and they should stop
	                    frontier.finish();
	                    for (BLCrawler crawler : crawlers) {
	                      crawler.onBeforeExit();
	                      Object object = crawler.getMyLocalData();
	                      if (object != null && object instanceof List<?>) {
	                    	  linkList.addAll((List<String>) object);
	                      }
	                    }
